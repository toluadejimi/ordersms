    ...
    <div class="mx-auto space-y-4">
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-md overflow-hidden border border-slate-200 dark:border-white/10">
            ...
            <div class="p-4 sm:p-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    ...
                    {{-- Country --}}
                    <div x-data="countrySelect()" x-init="init()">
                        <label class="block text-xs font-semibold text-slate-700 dark:text-zinc-300 mb-1.5">Country</label>
                        <div class="relative">
                            <button type="button" @click="toggleDropdown()"
                                class="w-full text-left text-sm px-3 py-2.5 rounded-lg border border-slate-300 dark:border-white/10 bg-white dark:bg-zinc-900 focus:border-blue-900 dark:focus:border-blue-500 flex items-center justify-between">
                                <span x-text="selectedLabel || 'Select country...'" class="text-slate-700 dark:text-zinc-300 truncate"></span>
                            ...
                            <input type="hidden" id="country-id" x-model="selectedValue">
                            <input type="hidden" id="country-name" x-model="selectedLabel">
                            <div x-show="isOpen" x-transition @click.away="isOpen=false"
                                ...
                                <div class="p-2 border-b border-slate-200 dark:border-white/10">
                                    <input type="text" x-model="searchQuery" @input="filterOptions()" placeholder="Search countries..."
                                ...
                                <div class="max-h-60 overflow-y-auto">
                                    <template x-for="o in filteredOptions" :key="o.value">
                                        <div @click="selectOption(o)"
                                            class="px-3 py-2 text-sm hover:bg-blue-50 dark:hover:bg-blue-900/30 cursor-pointer text-slate-700 dark:text-zinc-300"
                                            x-text="o.label"></div>
                                    </template>
                                    <div x-show="filteredOptions.length===0" class="px-3 py-4 text-sm text-center text-slate-500 dark:text-zinc-500">No countries found</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- Service --}}
                    <div x-data="serviceSelect()" x-init="init()">
                        <label class="block text-xs font-semibold text-slate-700 dark:text-zinc-300 mb-1.5">Service</label>
                        <div class="relative">
                            <button type="button" @click="if(!loading && options.length) toggleDropdown()"
                                ...
                                :class="{'opacity-50 cursor-not-allowed': loading || !options.length}">
                                <span class="text-slate-700 dark:text-zinc-300 truncate">
                                    <template x-if="loading">
                                        <span class="flex items-center gap-2">
                                            <svg class="animate-spin h-3.5 w-3.5 text-blue-900 dark:text-blue-400" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
                                            Loading services...
                                        </span>
                                    </template>
                                    <template x-if="!loading">
                                        <span x-text="selectedLabel || (options.length ? 'Select service...' : 'Choose country first')"></span>
                                    </template>
                                </span>
                                <svg class="w-4 h-4 text-slate-400 flex-shrink-0 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                            </button>
                ...
                <div id="priceBox" class="bg-blue-50 dark:bg-zinc-900/60 border-2 border-dashed border-blue-200 dark:border-white/10 rounded-lg py-4 px-4 text-center mb-4">
                    <p class="text-xs text-slate-600 dark:text-zinc-400">Select country &amp; service to load available numbers</p>
                </div>
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-md overflow-hidden border border-slate-200 dark:border-white/10">
            <div class="bg-gradient-to-r from-blue-900 to-blue-800 dark:from-zinc-700 dark:to-zinc-600 px-4 sm:px-6 py-3 flex items-center justify-between">
                ...
                <span class="text-xs text-blue-200 dark:text-zinc-400">—</span>
            </div>
                ...
                <div data-activation="{{ $o->activation_id }}" class="p-4 space-y-2.5">
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs font-mono text-slate-500 dark:text-zinc-400 mt-0.5">{{ $o->phone }}</p>
                            <p class="text-xs text-slate-400 dark:text-zinc-500 mt-0.5">🌍 {{ $o->country }}</p>
                        </div>
            ...
            <div class="hidden sm:block overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200 dark:divide-white/5">
                    <thead class="bg-slate-50 dark:bg-zinc-900/50">
                        <tr>
                            @foreach(['Service','Country','Number','Price (₦)','Status','Code','Date','Action'] as $col)
                    ...
                    <tbody class="bg-white dark:bg-zinc-800 divide-y divide-slate-200 dark:divide-white/5">
                        ...
                        <tr data-activation="{{ $o->activation_id }}" class="hover:bg-slate-50 dark:hover:bg-zinc-700/50 transition-colors">
                            ...
                            <td class="px-4 py-3 text-sm text-slate-600 dark:text-zinc-400">{{ $o->country }}</td>
                            <td class="px-4 py-3 text-sm font-mono text-slate-600 dark:text-zinc-400">{{ $o->phone }}</td>
...
function countrySelect() {
    return {
        options: @json(collect($countries)->map(fn($c) => ['value' => $c['id'], 'label' => $c['name']])->values()),
        filteredOptions: [], isOpen: false, selectedValue: '', selectedLabel: '', searchQuery: '',
        init() { this.filteredOptions = this.options; },
        toggleDropdown() { this.isOpen = !this.isOpen; if (this.isOpen) { this.searchQuery = ''; this.filteredOptions = this.options; } },
        filterOptions() { const q = this.searchQuery.toLowerCase(); this.filteredOptions = this.options.filter(o => o.label.toLowerCase().includes(q)); },
        selectOption(o) {
            this.selectedValue = o.value; this.selectedLabel = o.label; this.isOpen = false;
            loadServices(o.value);
        }
    };
}
...
function serviceSelect() {
    return {
        options: [], filteredOptions: [], isOpen: false,
        selectedValue: '', selectedLabel: '', searchQuery: '', loading: false,
        init() { window._serviceAlpine = this; },
        setOptions(list) { this.options = list; this.filteredOptions = list; this.selectedValue = ''; this.selectedLabel = ''; this.loading = false; },
        setLoading(v) { this.loading = v; if (v) { this.isOpen = false; } },
        toggleDropdown() { this.isOpen = !this.isOpen; if (this.isOpen) { this.searchQuery = ''; this.filteredOptions = this.options; } },
        filterOptions() { const q = this.searchQuery.toLowerCase(); this.filteredOptions = this.options.filter(o => o.label.toLowerCase().includes(q)); },
        selectOption(o) {
            this.selectedValue = o.value; this.selectedLabel = o.label; this.isOpen = false;
            ...
            loadPrice(document.getElementById('country-id').value, o.value);
        }
    };
}
...
async function loadServices(countryId) {
    resetPrice();
    if (!window._serviceAlpine) return;
    window._serviceAlpine.setLoading(true);
    window._serviceAlpine.setOptions([]);

    try {
        const r = await fetch(`/virtual/services/${countryId}`);
        const j = await r.json();
        const list = (j.services || []).map(s => ({ value: s.code, label: s.name }));
        window._serviceAlpine.setOptions(list);
    } catch {
        window._serviceAlpine.setLoading(false);
    }
}

// ── Load price & render pool cards ────────────────────────────────────────
async function loadPrice(countryId, serviceCode) {
    resetPrice();
    ...
    try {
        const r = await fetch('/virtual/prices', {
            method: 'POST',
...
async function confirmPurchase() {
    ...
    if (!pool || !ngn || !serviceCode || !countryId) {
        Swal.fire({ icon: 'warning', title: 'Select a route', text: 'Please select a country, service and route first.', confirmButtonColor: '#1e3a8a', ...swalTheme() });
        return;
    }
    ...
    const result = await Swal.fire({
        ...
        html: `<p class="text-sm" style="color:${isDark()?'#a1a1aa':'#64748b'}">You are about to purchase a number for</p>
               ...
               <p class="text-sm" style="color:${isDark()?'#a1a1aa':'#64748b'}">in <strong>${countryName}</strong></p>
               ...
               <p class="text-xs mt-3" style="color:${isDark()?'#71717a':'#94a3b8'}">This amount will be deducted from your wallet immediately.</p>`,
        icon: 'question',
    ...
    try {
        const r = await fetch('/virtual/purchase', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': CSRF, 'Accept': 'application/json' },
            body: JSON.stringify({ country: countryId, service: serviceCode, service_name: serviceName, pool: parseInt(pool) }),
        });
        const j = await r.json();
        ...
        const d = j.data;
        prependOrderRow(d);
        // Register with the global batch poller and start cancel countdown
        registerPending(d.activation_id);
        startCancelCountdown(d.activation_id, 180);
        ...
        Swal.fire({
            ...
            html: `<p class="text-sm" style="color:${isDark()?'#a1a1aa':'#64748b'}">Your number is ready. Waiting for SMS code.</p>
                   <div style="background:${isDark()?'#3f3f46':'#f1f5f9'};border-radius:8px;padding:10px 16px;margin-top:12px">
                       ...
                       <p style="font-size:11px;color:${isDark()?'#71717a':'#94a3b8'};margin-top:4px">${d.service} · ${d.country}</p>
                   </div>`,
...
function resetPrice() {
    document.getElementById('priceBox').innerHTML = `<p class="text-xs text-slate-600 dark:text-zinc-400">Select country &amp; service to load available numbers</p>`;
    document.getElementById('selected-pool').value = '';
    document.getElementById('selected-ngn').value  = '';
    document.getElementById('purchaseBtn').disabled = true;
}

// ── Prepend row after purchase ─────────────────────────────────────────────
function prependOrderRow(d) {
    ...
    if (tbody) {
        ...
        tr.innerHTML = `
            ...
            <td class="px-4 py-3 text-sm text-slate-600 dark:text-zinc-400">${d.country}</td>
        ...
        div.innerHTML = `
            <div class="flex items-start justify-between gap-3">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-slate-400 dark:text-zinc-500 mt-0.5">🌍 ${d.country}</p>
                </div>
...
// ══════════════════════════════════════════════════════════════════════════
// BATCH POLLER — one shared interval for ALL pending activations
// ══════════════════════════════════════════════════════════════════════════
const _pendingIds    = new Set();   // activation IDs currently being watched
let   _batchInterval = null;        // the single shared setInterval handle
...
function unregisterPending(id) {
    ...
    if (_pendingIds.size === 0 && _batchInterval) {
        clearInterval(_batchInterval);
        _batchInterval = null;
    }
}

function _ensureBatchPoller() {
    if (_batchInterval) return; // already running
    _batchInterval = setInterval(_doBatchPoll, POLL_MS);
}

async function _doBatchPoll() {
    ...
    try {
        const r = await fetch('/virtual/poll/batch', {
            method : 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': CSRF, 'Accept': 'application/json' },
        ...
        if (!r.ok) return; // network hiccup — retry next tick

        const j = await r.json();
        if (!j.success || !j.results) return;

        for (const [id, result] of Object.entries(j.results)) {
            if (result.status === 'done' && result.code) {
                // ── Stop watching this ID immediately ──────────────────────
                unregisterPending(id);

                // ── Stop its cancel countdown ──────────────────────────────
                if (_cancelTimers[id]) {
                ...
                Swal.fire({
                    ...
                    html: `
                        <div class="text-center">
                            <p style="font-size:13px;color:${isDark()?'#a1a1aa':'#64748b'};margin-bottom:12px">
                                Your verification code is:
                            </p>
                            <div style="background:${isDark()?'#3f3f46':'#f1f5f9'};
                ...
                // Provider reports cancelled/expired — stop watching silently
                unregisterPending(id);
                if (_cancelTimers[id]) { clearInterval(_cancelTimers[id]); delete _cancelTimers[id]; }
                delete _cancelDeadlines[id];
            }
            // 'active' / 'unknown' → keep polling
        }
    } catch {
...
// ── 3-minute cancel countdown (wall-clock accurate) ───────────────────────
const _cancelTimers    = {};
...
async function cancelOrder(id) {
    ...
    try {
        const r = await fetch(`/virtual/cancel/${id}`, {
            method: 'POST',
        ...
        if (j.success) {
            // Stop polling and countdown for this ID
            unregisterPending(id);
            if (_cancelTimers[id]) { clearInterval(_cancelTimers[id]); delete _cancelTimers[id]; }
            delete _cancelDeadlines[id];
...
// ── Auto-register active orders on page load ───────────────────────────────
...
(function() {
    ...
    const LOCK    = 180;
    const remaining = Math.max(0, LOCK - age);

    // Register with batch poller
    registerPending(id);

    if (remaining > 0) {
        startCancelCountdown(id, remaining);
    } else {
        // Lock already expired — enable cancel button immediately
        document.querySelectorAll(`[data-activation="${id}"] .cancel-btn`).forEach(btn => {
            btn.textContent = 'Cancel & Refund';
            btn.disabled = false;
            btn.classList.remove('opacity-50', 'cursor-not-allowed');
